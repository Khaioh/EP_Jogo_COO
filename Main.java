...
public class Main {
	...
	public static void main(String [] args){
		...
		double [] enemy3_angle = new double[10];				// ângulos (indicam direção do movimento)
		double [] enemy3_RV = new double[10];					// velocidades de rotação
		double [] enemy3_explosion_start = new double[10];		// instantes dos inícios das explosões
		double [] enemy3_explosion_end = new double[10];		// instantes dos finais das explosões
		long [] enemy3_nextShoot = new long[10];				// instantes do próximo tiro		
		double enemy3_spawnX = GameLib.WIDTH * 0.25;			// coordenada x do próximo inimigo tipo 3 a aparecer
		int enemy3_count = 0;									// contagem de inimigos tipo 3 (usada na "formação de voo")
		double enemy3_radius = 18.0;							// raio (tamanho aproximado do inimigo 3)
		long nextEnemy3 = currentTime + 5000;					// instante em que um novo inimigo 3 deve aparecer
		...
		/* variáveis dos projéteis lançados pelos inimigo tipo 3 */
		
		int [] e_projectile_states3 = new int[200];					// estados
		double [] e_projectile_X3 = new double[200];				// coordenadas x
		double [] e_projectile_Y3 = new double[200];				// coordenadas y
		double [] e_projectile_VX3 = new double[200];				// velocidade no eixo x
		double [] e_projectile_VY3 = new double[200];				// velocidade no eixo y
		double e_projectile_radius3 = 3.0;							// raio (tamanho dos projétil)
		...
		for(int i = 0; i < e_projectile_states3.length; i++) e_projectile_states3[i] = INACTIVE;
		...
		while(running){
			...
			if(player_state == ACTIVE){
				...
				for(int i = 0; i < e_projectile_states3.length; i++){
					
					double dx = e_projectile_X3[i] - player_X;
					double dy = e_projectile_Y3[i] - player_Y;
					double dist = Math.sqrt(dx * dx + dy * dy);
					
					if(dist < (player_radius + e_projectile_radius3) * 0.8){
				...
				for(int i = 0; i < enemy3_states.length; i++){
					...
					double dx = enemy3_X[i] - player_X;
					double dy = enemy3_Y[i] - player_Y;
					double dist = Math.sqrt(dx * dx + dy * dy);
					
					if(dist < (player_radius + enemy3_radius) * 0.8){
			...
			for(int i = 0; i < e_projectile_states3.length; i++){
				...
				if(e_projectile_states3[i] == ACTIVE){
					
					/* verificando se projétil saiu da tela */
					if(e_projectile_Y3[i] > GameLib.HEIGHT) {
						
						e_projectile_states3[i] = INACTIVE;
					}
					else {
					
						e_projectile_X3[i] += e_projectile_VX3[i] * delta;
						e_projectile_Y3[i] += e_projectile_VY3[i] * delta;
					}
				}
			}
			...
			for(int i = 0; i < enemy3_states.length; i++){
				...
				if(enemy3_states[i] == ACTIVE){
					...
					if(enemy3_Y[i] > GameLib.HEIGHT + 10) {
						...
						enemy3_X[i] += enemy3_V[i] * Math.cos(enemy3_angle[i] * (0.5)) * delta * (0.5);
						enemy3_Y[i] += enemy3_V[i] * Math.sin(enemy3_angle[i] * (-0.5)) * delta * (-1.0);
						enemy3_angle[i] += enemy3_RV[i] * delta;
						
							if(currentTime > enemy3_nextShoot[i] && enemy3_Y[i] < player_Y){
																							
								int free = findFreeIndex(e_projectile_states3);
							
								if(free < e_projectile_states3.length){
								
									e_projectile_X3[free] = enemy3_X[i];
									e_projectile_Y3[free] = enemy3_Y[i];
									e_projectile_VX3[free] = Math.cos(enemy3_angle[i]) * 0.45;
									e_projectile_VY3[free] = Math.sin(enemy3_angle[i]) * 0.45 * (-1.0);
									e_projectile_states3[free] = ACTIVE;
								
									enemy3_nextShoot[i] = (long) (currentTime + 200 + Math.random() * 500);
								}
							}
					}	
				}
			...
			if(currentTime > nextEnemy3){
				...
				if(free < enemy3_states.length){
					...
					if(enemy3_count < 10){
						...
						enemy3_count = 0;
						enemy3_spawnX = Math.random() > 0.5 ? GameLib.WIDTH * 0.2 : GameLib.WIDTH * 0.8;
						nextEnemy3 = (long) (currentTime + 3000 + Math.random() * 3000);
					}
				}
			}
			...
			/* desenhando projeteis (inimigo 3) */
			...
			for(int i = 0; i < e_projectile_states3.length; i++){
				...
				if(e_projectile_states3[i] == ACTIVE){
					...
					GameLib.setColor(Color.RED);
					GameLib.drawCircle(e_projectile_X3[i], e_projectile_Y3[i], e_projectile_radius3);
				}
			}
			...
			for(int i = 0; i < enemy3_states.length; i++){
				...
				if(enemy3_states[i] == ACTIVE){
					...
					GameLib.setColor(Color.RED);
					GameLib.drawDiamond(enemy3_X[i], enemy3_Y[i], enemy3_radius);
				}
			}
